...
class dict_en():
    ...
    def selectword(self, choice, count, select=True):
        ...
        if select:
            ...
            self.words = list(DICT.query_sql(sql))
        else:
            total_count = DICT.count()
            print(total_count, count)
            print(np.random.randint(1, high=total_count, size=count))
            if count > total_count:
                word_ids += np.random.randint(1,
                                              high = total_count, size=total_count).tolist()
                self.words = list(DICT.query_batch(word_ids))
        ...
        with open('./DICT/choice_en.pkl', 'w') as f:
            json.dump(choice,f)
    ...
    def __getwrong(self):
        wrongwords = []
        if os.path.exists('./DICT/wrongEN.pkl'):
            with open('./DICT/wrongEN.pkl', 'rb') as f:
                wrongwords = pickle.load(f)
        return wrongwords
    ...
    def addwrong(self, wordid):
        ...
        if wordid not in self.wrongwords:
            ...
            with open('./DICT/wrongEN.pkl', 'wb') as f:
